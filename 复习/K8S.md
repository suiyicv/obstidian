...
我们想查看所有容器的状态，但是我们能看到的所有数据都是从etcd的数据库里面来的，那谁帮我们去etcd数据库里面取数据呢？<font color="#f79646">apiserver这个组件</font>
(3)负责接收工作节点的注册请求
...
但是现在还存在网络通信的问题
主节点是一个单独的机器，工作节点也是一个单独的机器，这些容器是跨物理机分散存着的，我们可以通过flannel将网络打通，但是我们不用flannel，性能太垃圾，他所使用的网络规模是相对来说比较小的
...
给后端的业务添加健康状态检查，是为了让前端的负载均衡器通过某种方式能够检测这些业务是不是正常运行。如果没有健康状态检查，负载均衡器在接受请求的时候有可能把这个请求分发到故障的业务服务器上面，导致客户访问不正常，如果有健康状态检查，负载均衡器就不再往故障机器上分发请求
...
这种方式呢，被监控机上面不需要装然和的客户端，你只需要把你被监控机上边的url地址告诉我，我的监控服务器会自动联系你给我的url地址，然后自动的去采集监控数据，这种动态的监控方式主要用来监控k8s自身，监控k8s集群自身的核心组件。比如：kukelet是调用容器引擎来关联容器的生命周期的，单说kubelet本身就带有监控功能，其他的核心组件本身也都具有监控的功能，在他们运行起来的时候也都会，搜集监控数据，只不过没有zabbix或者Prometheus这种软件的配合，他的数据没有地方存放，所以通过服务发现的方式就可以实现对各组件的监控。但是配置起来需要你写大量的正则表达式去匹配你想要的数据
