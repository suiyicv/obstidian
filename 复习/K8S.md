...
这种方式呢，被监控机上面不需要装然和的客户端，你只需要把你被监控机上边的url地址告诉我，我的监控服务器会自动联系你给我的url地址，然后自动的去采集监控数据，这种动态的监控方式主要用来监控k8s自身，监控k8s集群自身的核心组件。比如：kukelet是调用容器引擎来关联容器的生命周期的，单说kubelet本身就带有监控功能
