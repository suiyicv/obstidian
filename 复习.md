...
## 四.部署K8s1.29集群
两种部署方式
二进制包的方式(非常提升个人能力)，整个集群里面的所有组件都需要手动安装，所有的组件都有相应的配置文件，所有的配置文件都需要手工编写，而且配置文件根本就没有，需要vim新建，一行一行手写，每个组件还需要手写启动脚本，k8s里面所有的组件之间通信，为了保证数据安全，都是需要证书的，生成证书用的还不是openssl 而是cfsl，第一你需要生成组件之间通信所需要的所有证书，真正生成证书的时候不报错，所有组件之间对接的时候检测到你的证书不对，才开始报错
