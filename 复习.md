...
| SGID | 2      |
| SBIT | 1      |
SUID:任何用户在运行拥有suid权限的<span style="background:#affad1">命令</span>(二进制可执行文件)时,都以该命令拥有者的身份执行
两种更改方式
chmod u+s 命令 
chmod 4755 命令
...
## 四.部署K8s1.29集群
环境规划
192.168.140.21 K8s-master     2u4G
192.168.140.22 K8s-node01    2u4G
192.168.140.23 K8s-node02    2u4G
