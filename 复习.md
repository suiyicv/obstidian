...
![[Pasted image 20240725155001.png]]
![[Pasted image 20240725155109.png]]


1.对文件设置了acl权限后，mask值产生，或刷新旧mask值。除非手动设置，否则mask值会首先满足最大facl中的权限。 
2.我们可以通过调整mask值来限制acl权限的大小范围，也会影响基本组权限（有效权限范围）。 setfacl -x m: : r 
3.mask值权限和facl权限重叠部分，为有效权限。（一般不更改） 4.mask值权限不仅会显示在getfacl查询表中，也会同步到，文件所属组权限位置。 
5.文件一旦设置了acl权限，需要使用getfacl来查看文件真实权限及acl权限。 
6.当删除单个acl之后，使用ll查看，发现在改文件或者目录不存在任何acl的情况下，仍然存在一个加号，显示存在acl设置，使用getfacl查看发现有mask值残留，使用命令setfacl -x m file ，删除之后再ll查看发现没有加号。 
7.文件设置acl权限后，chmod命令修改的是mask值，不是文件基本组权限。
...
相当于装载容器的箱子
实际情况：一个容器一个pod
![[Pasted image 20240725101912.png]]
![[Pasted image 20240725102033.png]]
指定镜像的下载策略
![[Pasted image 20240725102212.png]]
![[Pasted image 20240725102231.png]]
