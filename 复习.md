...
**仓库的类型**
公有仓库         私有仓库:企业级的应用
[Dockerhub](https://hub.docker.com/)  registry;harbor

构建私有仓库的方案
registry(早期镜像):就是一个容器，功能太少，仅支持上传下载
harbor(软件):VMware开源的;基于registry的改进版本;支持日志审计;权限分;统一认证
两种安装包:离线安装包(包含镜像)；在线安装包(内存小不包含镜像)
...
创建用户
给用户授权
![[Pasted image 20240722112416.png|350]]
![[Pasted image 20240722112437.png|550]]
授权
![[Pasted image 20240722112506.png|550]]

3.上传镜像
3.1 登录仓库
docker login # 回车默认登录dockerhub
docker login harbor.linux.com  # 需要添加解析
![[Pasted image 20240722112800.png]]
还是会产生一个错误
docker服务器默认走443端口，需要证书认证，需要再docker服务器上面
...
### harbor核心组件
单个harbor很容易成为单点故障
![[Pasted image 20240722150812.png|550]]
core services
 - ui
 - token
 - webhook(钩子函数) 和 regis'r
