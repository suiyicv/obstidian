...
```
### 4.4 修改文件所有者所属组
chown 命令可以修改所有者和所属组，作用于超级用户，普通用户不可用 chgrp  命令只可以修改所属组，作用于普通用户，修改所属组为自己的组
...
## 三.k8s创建容器的流程
创建容器的请求首先发给apiserver
apiserver接收到我们创建容器的请求，对我们进行身份验证，鉴权通过之后，他来接收我们创建容器的请求
apiserver拿到我们创建容器的请求之后，他肯定能够知道我们需要创建的容器的各种信息，然后他会把容器的创建信息写道etcd数据库里面去
比如存着：<span style="background:#affad1">容器名xxxx 镜像名xxx (创建请求)</span>
然后这个时候schedule这个调度会干什么呢？
他会周期性的询问apiserver你有没有请求需要我调度，apiserver接收到这个询问之后，他就回去etcd数据库里面去查，肯定能狗查到
数据库里面存在着一个创建请求，然后他会拿着这个请求返回告诉给这个调度
调度现在就知道了有一个创建的请求需要调度
然后就会按着一定的算法选择一个工作节点，选择到合适的工作节点之后，scheduler会把这个工作节点的ip和端口告诉apiserver
说我帮你选择一个容器，apiserver拿到这个工作节点的ip等信息后，又会把这些信息存放到etcd数据库里面，会跟之前的创建请求形成一个对应关系
<span style="background:#affad1">容器名xxxx 镜像名xxx (创建请求)</span><span style="background:rgba(136, 49, 204, 0.2)">--->node02</span>

工作节点的kubelet会不停的找apiserver去问，你这边有没有需要我真正帮你创建的容器，apiserver接收到询问之后，又回去etcd库里面去查有没有 <span style="background:#affad1">容器名xxxx 镜像名xxx (创建请求)</span><span style="background:rgba(136, 49, 204, 0.2)">--->node02</span>这样子的创建关系存在，查到相应的信息之后，会把这个信息返回给kubelet
kubelet拿到apiserver返回的信息之后
这个时候kubelet才会调用底层的Docker Engine 最终把这个容器创建出来
创建出来之后kubelet再把建好的容器信息返回给
