...
(3)支持服务发现和负载均衡
自动做负载均衡
四个容器部署好了以后，跑一样的业务，传统我们肯定要通过haproxy或者ngxin做负载均衡，但是有了K8s以后， zhi'yao
kubernetes和docker有什么区别

如果坏的是数据库呢?
最重要的是数据，K8s针对数据库挂了，会自动创建了新的数据库容器，然后自动连接数据存储设备
...
master节点：负责正整个集群的管理操作(监控，状态维护，扩容，缩容)
node节点：至少一个node节点(工作节点)，允许容器的节点
etcd节点： 高性能的键值对数据库，作为K8s集群的后台数据库使用，存储集群的所有数据
etcd数据库怎么做备份(以快照的方式 )？
kubectl：客户端命令行工具
...
kubelet  负责调用工作节点的容器引擎进行整个生命周期管理
面试必聊
