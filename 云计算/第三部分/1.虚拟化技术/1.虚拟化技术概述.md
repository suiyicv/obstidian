...
完整的操作系统包含
	内核
	应用程序
也是通过底层的物理机创建出一些列的虚机供我们使用，但是核心的区别就是，基于操作系统的虚拟化，他创建出来的虚拟机，第一没有自己独立的虚拟硬件，第二，基于系统级别的虚拟化创建出来的虚拟机，你可以理解为他创建出来的就是一堆应用层的命令(应用程序)，他并不是一个完整的操作系统，没有独立的内核。假设我们拿一redis的镜像去创建一个虚拟机，这个虚拟机中只有和redis相关的指令(<span style="background:#affad1">bin</span>)和redis运行必备的一些依赖(<span style="background:#affad1">lib</span>)
## 2.根据Hypervisor的部署方式
原生虚拟化
