...
我们去创建交换机的时候如果我们不对他的网络做任何的修改的话，我们创建出来的所有虚拟机默认全都会自动接入到这个叫default的虚拟网络上，这就相当于现实生活中你弄了两台电脑通过网线把它接到了交换机上，然后这个默认网络还自带dhcp功能，由于我们的系统安装好之后，他那个网卡默认的就是dhcp获取ip的，
这个路由是在谁和谁之间转数据的呢？
